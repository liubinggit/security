...
public class SmsCodeFilter extends OncePerRequestFilter implements InitializingBean {
	...
	public void afterPropertiesSet() throws ServletException {
		...
		String[] urlStrs = {};
		if(!"".equals(securityProperties.getCode().getSms().getUrl())) {
			securityProperties.getCode().getSms().getUrl().split(",");
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		log.info("SmsCodeFilter doFilterInternal");
		log.info("SmsCodeFilter url:" + request.getRequestURI());
		log.info("SmsCodeFilter method:" + request.getMethod());
		log.info("SmsCodeFilter urls:" + urls);
		...
		log.info("SmsCodeFilter 校验url是否需要验证验证码:" + action);
		if (action) {
			try {
	...
	private void validate(ServletWebRequest request) throws ServletRequestBindingException {
		...
		log.info("SmsCodeFilter smsCode:" + codeInRequest);
		...
		log.info("SmsCodeFilter validate 验证通过");
		sessionStrategy.removeAttribute(request, ValidateCodeProcessor.SESSION_KEY_PREFIX+"SMS");
	}
